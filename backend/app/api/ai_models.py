...
class AIConversionRequest(BaseModel):
    ...
    mode: Literal["auto", "speed", "balanced", "quality", "max_quality"] = Field(
        default="auto", description="Quality mode — 'auto' lets AI decide"
    )
    ...
    enable_ai_preprocessing: bool = Field(
        default=True, description="Enable smart preprocessing (denoise, contrast, sharpen)"
    )
    enable_sam: bool = Field(
        default=True, description="Enable SAM-guided segmentation for complex images"
    )
    enable_optimization: bool = Field(
        default=True, description="Enable DiffVG-inspired SVG post-optimization"
    )
    enable_gradients: bool = Field(default=True, description="Detect and add SVG gradient fills")
    enable_upscale: bool = Field(default=True, description="Auto-upscale small images")
    enable_bg_removal: bool = Field(
        default=False, description="Remove background before conversion"
    )

    # Custom parameters
    custom_params: Optional[Dict[str, Any]] = Field(
        default=None, description="Custom engine parameters to override AI suggestions"
    )

    # Options
    webhook_url: Optional[str] = Field(
        default=None, description="Webhook URL for conversion completion notification"
    )
    generate_preview: bool = Field(
        default=True, description="Generate a preview image alongside the SVG"
    )
