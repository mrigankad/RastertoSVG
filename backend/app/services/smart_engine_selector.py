...
class SmartEngineSelector:
    ...
    def analyze_and_select(
        ...
        alt_engine, alt_confidence = self._get_alternative(
        ...
        reasoning = self._build_reasoning(
            category, engine, features, category_confidence
        )
    ...
    def _extract_color_features(
        self, image: np.ndarray, features: ImageFeatures
    ) -> None:
        ...
        features.large_uniform_regions = int(
            np.sum(stats[1:, cv2.CC_STAT_AREA] > large_threshold)
        )
        small_threshold = total_pixels * 0.001
        features.small_detail_regions = int(
            np.sum(stats[1:, cv2.CC_STAT_AREA] < small_threshold)
        )
    ...
    def _select_engine(
        ...
        engine, confidence = category_engines.get(
            category, (EngineType.VTRACER, 0.60)
        )
    ...
    def _estimate_time(
        self, engine: EngineType, features: ImageFeatures
    ) -> float:
    ...
    def _build_reasoning(
        ...
        reasons.append(
            f"Image classified as '{category.value}' "
