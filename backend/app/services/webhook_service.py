...
class WebhookService:
    ...
    async def trigger_event(
        ...
        for webhook in webhooks:
            if event in webhook.events:
                ...
                import asyncio
                asyncio.create_task(
                    self.send_webhook(webhook, event, data, job_id, batch_id)
                )
    
    async def trigger_conversion_started(
        self,
        job_id: str,
        file_id: str,
        ...
        await self.trigger_event(
            'conversion.started',
            {'file_id': file_id, 'options': options},
            job_id=job_id
        )
    
    async def trigger_conversion_progress(
        self,
        job_id: str,
        progress: float,
        stage: str
    ...
    async def trigger_conversion_completed(
        self,
        job_id: str,
        result: Dict[str, Any]
    ):
    ...
    async def trigger_conversion_failed(
        self,
        job_id: str,
        error: str
    ):
    ...
    async def trigger_batch_completed(
        self,
        batch_id: str,
