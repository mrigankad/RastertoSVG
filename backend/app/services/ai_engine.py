        ...
        try:
            ...
            result["engine_recommendation"] = {
                ...
                "alternative": (
                    ...
                    if recommendation.alternative_engine
                    else None
                ),
            ...
            if actual_engine in (EngineType.SAM_VTRACER, EngineType.SAM_DIFFVG):
                if sam_masks and len(sam_masks) > 0:
                    ...
                    conversion_result = self._convert_vtracer(processed_image, output_path, params)
            elif actual_engine == EngineType.POTRACE:
                conversion_result = self._convert_potrace(processed_image, output_path, params)
            else:
                conversion_result = self._convert_vtracer(processed_image, output_path, params)
            ...
            try:
                result = self._fallback_convert(input_path, output_path, result)
            except Exception as fallback_error:
                result["fallback_error"] = str(fallback_error)

        return result

    def _apply_sam_segmentation(self, image: np.ndarray) -> Optional[List[Dict[str, Any]]]:
        ...
        try:
            if self.sam_vectorizer and self.sam_vectorizer.is_available():
                ...
                if masks:
                    ...
                    h, w = image.shape[:2]
                    min_area = h * w * 0.01
                    significant = [m for m in masks if m.get("area", 0) > min_area]
    ...
    def _convert_with_sam(
        ...
        for i, mask_info in enumerate(sorted_masks[:15]):  # Max 15 regions
            ...
            if len(region.shape) == 3:
                for c in range(3):
                    region[:, :, c] = np.where(region_mask > 128, region[:, :, c], 255)
            ...
            try:
                ...
                self.vtracer.convert_pillow(pil_img, tmp_out_path, **vtracer_params)
